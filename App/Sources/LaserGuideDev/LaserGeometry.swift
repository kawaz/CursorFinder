...
//
// 2026-07-10 実機フィードバック #1 / #3 / #4 の修正で新設。DR-0013 (2026-07-13) で
// #3 の standoff 決定を「距離比率 + min/max クランプ」→「px 固定値」に簡素化:
//
//   #1 「各モニタは自分の 4 角からのレーザーだけ描く」
//     → 四隅の列挙は自ディスプレイの logicalBounds のみを対象にする。
//        他ディスプレイの角からの延長線は描画しない。
//
//   #3 「ポインタ手前で止まる三角形」(DR-0013 で頂点 1 点集約)
//     → corner (幅 cornerHalfWidth*2) が底辺、target から standoff px 手前が頂点 (1 点)。
//        `taperApexPoint` は「ポインタから standoff だけ corner 側に戻した点」を返す純粋幾何関数。
//        DR-0013: 「距離比率 + min/max クランプ」を廃止し、standoff は SettingsStore
//        `laserStandoffPx` (px 固定値、既定 40) を呼び出し側で直接渡す形に統一。
//
