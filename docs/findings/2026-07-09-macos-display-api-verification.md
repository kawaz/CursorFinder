...
### 5. カドなぞり・同一エッジ内ブロック/通過境目の観測 (2026-07-10、kawaz 手動実行 120 秒 / 5960 イベント)

構成は §4 と同じ (内蔵 2056x1329 + LG が上 (-258,-1440)-(3182,0)、継ぎ目 y=0 は x∈[0,2056] のみ通過可)。

**5a. max 側クランプの −0.02 インセットは普遍 (scale 1.0 でも、両軸同時でも各軸独立)**

```
LG 下辺ブロック区間 (x<0):   loc=(-77.57,-0.02) ... (-13.09,-0.02)  ← y = 0 − 0.02 (LG の max-y 側)
LG 右下カド:                loc=(3181.98,-0.02)                    ← x = 3182−0.02 かつ y = 0−0.02 (両軸同時)
内蔵 右下カド:              loc=(2055.98,1328.98)                  ← 両軸 max−0.02
LG 右上カド:                loc=(3181.98,-1440.00)                 ← x は max−0.02、y は min ちょうど
LG 左上カド:                loc=(-258.00,-1440.00)                 ← 両軸 min ちょうど
```

含意: インセットは Retina (scale 2.0) 固有ではない。**カドでは軸ごとに独立にクランプ**される (min 側 = 境界ちょうど / max 側 = −0.02 の規則が軸別に適用)。BX 判定・カドの両軸同時 BX は per-axis の ε 近傍判定で扱える。

**5b. 継ぎ目 (通過可能境界) の通過で「境界値ちょうどのイベント」は保証されない**

y=0 の継ぎ目を低速で往復しても y は 15.23 → −3.20 のように 0 を跨いで跳び、y=0.00 ちょうどのイベントは観測されなかった。→ **PX 検出は「連続イベント間の所属モニタ変化」のみで行う** (境界上イベントを期待する設計は不可)。DR-0004 の 2 世代履歴方式の裏付け。

**5c. 同一エッジ内のブロック区間→通過区間の境目は位置ベースで自然に切り替わる**

LG 下辺ブロック区間 (x=−13.09, y=−0.02 にクランプ中) から右へスライドして x>0 (通過区間) に入った直後のイベントで (1.00, 1.49) と内蔵側へ通過した。境目での引っかかり・異常イベントなし。→ DR-0006 のセグメント区間判定は交点位置の区間所属だけで実装できる。

**5d. カド越しの斜め通過も引っかかりなし**

内蔵左辺 (ブロック) を x=0.00 クランプのまま上へスライドし、y<0 に達した瞬間 (0.00,2.26)→(−14.50,−15.58) と LG へ斜め通過。カド (0,0) でのスタックは観測されず。

**5e. カドに押し付け続けると両軸の delta が流れ続ける**

LG 右下カド (3181.98,−0.02) に押し付け中、deltaX=+1〜+5 / deltaY=+1〜+3 が継続。両軸 BX の同時成立が持続観測できる → カドでの BP 発火判定は DR-0006 のカド優先度規則 (優先度リスト順) で一意化する前提が実機と整合。
