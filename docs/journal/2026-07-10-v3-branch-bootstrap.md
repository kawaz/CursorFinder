...
## 後半 (実機フィードバック反映後の機能拡張)

2 巡目の対処 (#4 の確定原因、standoff 割合ベース化、60Hz 合流、メニュー文言の利用者語彙化) を
反映した後、DR-0007 の完遂と新規要望 2 件への着手が進んだ。

### DR-0007 完遂: Persistence 純関数 → App 配線

`PersistedWorkspaceV3` / v1 migration / reconcile の Core 純関数実装 (`b20daaf`) に続き、
App 側の配線を実装 (`5b4aec2`):

- 起動時ロード順を v3 キー → v1 キー (migration) → いずれもなし (`AppState.initial`) の順に固定
- v3 が見つかった場合、永続化された translate/rotation はそのまま使い、scale だけは現在の
  実機構成 (`DisplaySnapshotProvider` が返す実測 mm/px) から再導出する。保存時点のディスプレイ
  構成と起動時の実機構成がずれていても scale だけは常に最新に追従する設計
- migration 実装時に「物理 mm 空間の軸向き」が DR-0005 に未規定だったことが判明 (`9ab039b`
  で追記)。論理座標と同じ y-down 規約に統一し、v1 (y-up 相当) からの変換で Y 軸反転補正を
  追加 (`cc61fba`)
- `.temporary` プレフィックスの一時キーを起動完了後に掃除する処理を追加 (migration 中間状態が
  永続化領域に残り続けないように)

### DR-0008 Phase 1: キャリブレーション UI

Web プロトタイプ (前身の実験リポ) の設計を踏襲し、WKWebView 純 view として実装:

- `RenderModel` (`a8215eb`): Core に表示専用の派生型を新設。ドメインモデル (`AppState`) を
  UI 都合で汚さず、pose/bounds から画面表示用の座標・ラベルを導出する層として分離
- キャリブレーション画面 (`b8c205f`): 物理配置 (pose の translate/rotation) をドラッグで
  編集し、確定操作で `AppState` に反映 → DR-0007 の永続化経路に乗る
- browser 単体での開発モード用に mock ブリッジを用意し、agent-browser での UI 検証を実施済み
  (`docs/runbooks` に開き方と検証観点を追記、`7633ba7`)
- persist effect の payload を `PersistedWorkspaceV3` 一本に統合 (`bf68c41`): それまで
  複数箇所に分散していた保存経路を単一の型に集約

### 新規要望 2 件 (kawaz、リモートから)

作業中に kawaz からリモートで 2 件の要望が入り、issue 起票 (`6b7f8eb`, `9f3989c`) の上で
両方とも着手・wip まで進めた:

- **プレゼンテーションモード** (`fecd1f3`): 画面共有・登壇時にレーザー/ワープ演出を見せたい/
  隠したい場面向け。キャプチャ表示トグル (画面共有ソフトに映すかどうか) + クリック可視化
  サークル (クリック位置を一時的に円で示す) を実装済み (wip)
- **フォーカスフラッシュ = 第3コンポーネント** (`c838a4a` で DR-0009 起草): レーザー
  (カーソル位置)・ワープ (カーソル移動補助) と並ぶ「いまどこ?」可視化コンポーネントとして、
  ウィンドウフォーカス変更時にフォーカス先モニタの縁を光らせる機能。段階実装 A (モニタ縁
  ハイライト) → B (ウィンドウ枠) の方針で、Phase A を実装済み (`7523579`, wip)。
  実装中に AX (Accessibility) 経由で取得するウィンドウフレーム座標が実測で y-down だったこと
  が判明し、DR-0009 の y-up 前提とズレている点を observer 実装のコメントに記録した。実機確認後
  に DR 側の記述を訂正する候補として残している

### 実機確認待ちキュー

- キャリブレーション UI の往復 (ドラッグ編集 → 確定 → 永続化 → 再起動後の再現)
- プレゼンテーションモードの画面共有ソフトへの映り込み、クリックサークルの見た目
- フォーカスフラッシュの発火感 (アプリ切替・ウィンドウ切替・別モニタ切替の 3 系)
- tap レイテンシの実測値 (メニュー `LG → Dump tap latency` の n/p50/p95/p99/max)

### 次

リリース統合 (xcodeproj への組み込み、CD への乗せ方) の方式調査が進行中。現状は
Core 102 テスト + App 40 テストが green (実行確認済み)。
